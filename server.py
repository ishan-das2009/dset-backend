...
async def login(login_data: UserLogin, request: Request):
    """Login with ID and mobile number (for students)"""
    normalized_mobile = normalize_mobile(login_data.mobile)
    
    user = await db.users.find_one({
        "id": login_data.id.strip()
    }, {"_id": 0})
    ...
    # Get quiz progress
    progress = await db.quiz_progress.find_one({"user_id": user["id"]}, {"_id": 0})
...
async def admin_login(login_data: AdminLogin, request: Request):
    """Login with ID and password (for admin)"""
    user_id = login_data.id.strip().upper()
    ...
    admin_user = await db.users.find_one({"id": user_id}, {"_id": 0})
...
async def get_me(current_user: dict = Depends(get_current_user)):
    ...
    progress = await db.quiz_progress.find_one({"user_id": current_user["id"]}, {"_id": 0})
    cert = await db.certificates.find_one({"user_id": current_user["id"]}, {"_id": 0})
...
async def verify_auth(current_user: dict = Depends(get_current_user)):
    ...
    return {"valid": True, "id": current_user["id"], "role": current_user.get("role")}
...
async def get_quiz_progress(current_user: dict = Depends(get_current_user)):
    ...
    if not progress:
        ...
        progress = QuizProgress(user_id=current_user["id"]).model_dump()
...
async def generate_certificate(current_user: dict = Depends(get_current_user)):
    ...
    if existing_cert:
        ...
        await log_activity(current_user["id"], current_user["name"], "certificate_download", {
    ...
    await log_activity(current_user["id"], current_user["name"], "certificate_generate", {
...
async def chat_with_bot(
    ...
    session_id = chat_data.session_id or f"{current_user['id']}-{uuid.uuid4().hex[:8]}"
